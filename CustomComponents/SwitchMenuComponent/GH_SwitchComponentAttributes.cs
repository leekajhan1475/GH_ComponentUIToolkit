...
{
    public class GH_SwitchComponentAttributes : GH_ComponentAttributes
    {
        private int offset;
        ...
        private GH_MenuCollection _composedCollection;
        ...
        /// <summary>
        /// Gets or set the visible item count after menu is expanded
        /// </summary>
        public int VisibleItemCount
        {
            get => this._visibleItemCount;
            private set => this._visibleItemCount = value;
        }

        public GH_SwitchComponentAttributes(GH_SwitchComponent component)
            : base(component)
        {
            collection = new GH_MenuCollection();
            this._composedCollection = new GH_MenuCollection();
            this._composedCollection.Merge(collection);
            CreateUnitDropDown();
        ...
        {
            GH_SwitchComponent gH_SwitcherComponent = (GH_SwitchComponent)base.Owner;
        ...
        {
            GH_SwitchComponent SwitchComponent = (GH_SwitchComponent)base.Owner;

            this._composedCollection = new GH_MenuCollection();

            EvaluationUnit activeUnit = SwitchComponent.ActiveUnit;

            if (activeUnit != null && activeUnit.Context.Collection != null)
            {
                this._composedCollection.Merge(SwitchComponent.ActiveUnit.Context.Collection);
            }
            if (collection != null)
            {
                this._composedCollection.Merge(collection);
            ...
            {
                this._composedCollection.Merge(unitMenuCollection);
            }
        }
        ...
        protected void CreateUnitDropDown()
        {
            ...
            if (gH_SwitcherComponent.EvalUnits != null && gH_SwitcherComponent.EvalUnits.Count != 0 && (gH_SwitcherComponent.EvalUnits.Count != 1 || gH_SwitcherComponent.UnitlessExistence))
            {
                ...
                this._visibleItemCount = gH_SwitcherComponent.VisibleItemCount;
                _UnitDrop.VisibleItemCount = this.VisibleItemCount;
                ...
                // Expand the menu upon placing the component of gh canvas?
        ...
        private void _UnitDrop__valueChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                MenuItem entry = menuDropDown.Items[menuDropDown.Value];
                if (entry.Data != null)
                {
                    EvaluationUnit evaluationUnit = (EvaluationUnit)entry.Data;
                    ((GH_SwitchComponent)base.Owner).SwitchUnit(evaluationUnit.Name);
                }
                else
                {
                    ((GH_SwitchComponent)base.Owner).ClearUnit();
                }
            }
            catch (Exception ex)
            {
        ...
        {
            EvaluationUnit activeUnit = ((GH_SwitchComponent)base.Owner).ActiveUnit;
            ComposeMenu();
            if (activeUnit != null)
            {
                if (_UnitDrop != null)
                {
                    int num = _UnitDrop.FindItemIndex(activeUnit.Name);
                    if (num != -1)
                    {
                        _UnitDrop.Value = num;
                    }
            ...
            else if (((GH_SwitchComponent)base.Owner).UnitlessExistence && _UnitDrop != null)
            {
                _UnitDrop.Value = 0;
            }
        }
        ...
        protected void LayoutBaseComponent()
        {
            ...
            float width = this._composedCollection.GetMinLayoutSize().Width;
        ...
        {
            GH_SwitchComponent gH_SwitcherComponent = (GH_SwitchComponent)owner;
            int num = 24;
            int num2 = 0;
            int num3 = 0;
            foreach (IGH_Param componentInput in gH_SwitcherComponent.StaticData.GetComponentInputs())
            {
                ...
                num2 = Math.Max(num2, GH_FontServer.StringWidth(componentInput.NickName, StandardFont.Standard));
            }
            ...
            foreach (IGH_Param componentOutput in gH_SwitcherComponent.StaticData.GetComponentOutputs())
            {
                ...
                num4 = Math.Max(num4, GH_FontServer.StringWidth(componentOutput.NickName, StandardFont.Standard));
            }
        ...
        public RectangleF LayoutComponentBox2(IGH_Component owner)
        {
            ...
            {
                val = Math.Max(val, GH_Convert.ToSize((SizeF)GH_FontServer.MeasureString(owner.NickName, StandardFont.Large)).Width + 6);
            }
        ...
        public void LayoutInputParams2(IGH_Component owner, RectangleF componentBox, int add_offset)
        {
            ...
            foreach (IGH_Param componentInput in gH_SwitcherComponent.StaticData.GetComponentInputs())
            {
                ...
                num = Math.Max(num, GH_FontServer.StringWidth(componentInput.NickName, StandardFont.Standard));
            }
        ...
        public void LayoutOutputParams2(IGH_Component owner, RectangleF componentBox, int add_offset)
        {
            ...
            foreach (IGH_Param componentOutput in gH_SwitcherComponent.StaticData.GetComponentOutputs())
            {
                ...
                num2 = Math.Max(num2, val);
                num = Math.Max(num, GH_FontServer.StringWidth(componentOutput.NickName, StandardFont.Standard));
            }
            num = Math.Max(num + 6, 12);
            num += num2 + add_offset;
            float num3 = componentBox.Height / (float)count;
        ...
        protected override void Layout()
        {
            this.Pivot = ((PointF)GH_Convert.ToPoint(this.Pivot));
            LayoutBaseComponent();
            GH_SwitchComponent gH_SwitchComponent = (GH_SwitchComponent)base.Owner;
            List<ExtendedPlug> inputs = new List<ExtendedPlug>();
            List<ExtendedPlug> outputs = new List<ExtendedPlug>();
            this._composedCollection.GetMenuPlugs(ref inputs, ref outputs, onlyVisible: true);
        ...
        {
            GH_SwitchComponent obj = (GH_SwitchComponent)base.Owner;
            float num = 0f;
            int num2 = 0;
            ...
            for (int i = 0; i < this._composedCollection.Menus.Count; i++)
            {
                float num4 = -1f;
                float num5 = 0f;
                bool expanded = this._composedCollection.Menus[i].IsExpanded;
                if (expanded)
                {
                    num4 = num3 + this._composedCollection.Menus[i].Height;
                    num5 = Math.Max(this._composedCollection.Menus[i].Inputs.Count, this._composedCollection.Menus[i].Outputs.Count) * 20;
                }
                else
                {
                    num4 = num3 + 5f;
                ...
                List<ExtendedPlug> inputs = this._composedCollection.Menus[i].Inputs;
                ...
                num3 += this._composedCollection.Menus[i].TotalHeight;
            }
        }
        ...
        public void LayoutMenuOutputs(RectangleF componentBox)
        {
            ...
            for (int i = 0; i < this._composedCollection.Menus.Count; i++)
            {
                ...
                List<ExtendedPlug> outputs = this._composedCollection.Menus[i].Outputs;
        ...
        private void LayoutMenuCollection()
        {
            ...
            this._composedCollection.Style = impliedStyle;
            this._composedCollection.Palette = impliedPalette;
            this._composedCollection.Layout();
        }
        ...
        protected void LayoutMenu()
        {
            ...
            this._composedCollection.Pivot = new PointF(this.Bounds.X, (int)this.Bounds.Y + offset);
            this._composedCollection.Width = this.Bounds.Width;
            LayoutMenuCollection();
            this.Bounds = (new RectangleF(this.Bounds.X, this.Bounds.Y, this.Bounds.Width, this.Bounds.Height + this._composedCollection.Height));
        }
        ...
        protected override void Render(GH_Canvas iCanvas, Graphics graph, GH_CanvasChannel channel)
        {
            ...
            //    {
            //        // Render this switch component's "Base"
            //        this.RenderComponentCapsule2(iCanvas, graph);

            //        // Sets the runtime status
            //        this._composedCollection.RuntimeStatus = (int)this.Owner.RuntimeMessageLevel;
            //        // Sets the select status
            //        this._composedCollection.IsSelected = this.Selected;
            //        // Sets the locked status
            //        this._composedCollection.IsLocked = this.Owner.Locked;
            //        // Perform render operation to the collection of menus.
            //        this._composedCollection.Render(new WidgetRenderArgs(iCanvas, WidgetChannel.Object));
            //    }
            //}
            //else
            //{
        ...
        {
            this._composedCollection.Render(new WidgetRenderArgs(sender, WidgetChannel.Overlay));
        }

        protected void RenderComponentCapsule2(GH_Canvas canvas, Graphics graphics)
        {
            RenderComponentCapsule2(canvas,
                                    graphics, 
                                    drawComponentBaseBox: true,
                                    drawComponentNameBox: true,
        ...
        protected void RenderComponentCapsule2(GH_Canvas canvas,
                                               Graphics graphics,
                                               ...
                                               bool drawZuiElements)
        {
            ...
            bool flag = base.Owner.Params.Input.Count == 0;
            bool flag2 = base.Owner.Params.Output.Count == 0;
            ...
            {
                foreach (IGH_Param staticInput in gH_SwitchComponent.StaticData.StaticInputs)
                {
                    val2.AddInputGrip(staticInput.Attributes.InputGrip.Y);
                }
                foreach (IGH_Param dynamicInput in gH_SwitchComponent.StaticData.DynamicInputs)
                {
                    val2.AddInputGrip(dynamicInput.Attributes.InputGrip.Y);
                }
                foreach (IGH_Param staticOutput in gH_SwitchComponent.StaticData.StaticOutputs)
                {
                    val2.AddOutputGrip(staticOutput.Attributes.OutputGrip.Y);
                }
                foreach (IGH_Param dynamicOutput in gH_SwitchComponent.StaticData.DynamicOutputs)
                {
            ...
            else
            {
                ...
                {
                    GH_Capsule obj = GH_Capsule.CreateTextCapsule(base.m_innerBounds, 
                                                                  base.m_innerBounds, 
                                                                  GH_Palette.Black, 
                                                                  base.Owner.NickName,
                                                                  StandardFont.Large, 
        ...
        public static void RenderComponentParameters2(GH_Canvas canvas, Graphics graphics, IGH_Component owner, GH_PaletteStyle style)
        {
            ...
            if (zoomFadeLow >= 5)
            {
                ...
                foreach (IGH_Param staticInput in gH_SwitcherComponent.StaticData.StaticInputs)
                {
                    ...
                    if (bounds.Width >= 1f)
                    {
                        graphics.DrawString(staticInput.NickName, StandardFont.Standard, solidBrush, bounds, farCenter);
                ...
                foreach (IGH_Param staticOutput in gH_SwitcherComponent.StaticData.StaticOutputs)
                {
                    ...
                    if (bounds2.Width >= 1f)
                    {
                        graphics.DrawString(staticOutput.NickName, StandardFont.Standard, solidBrush, bounds2, farCenter);
        ...
        {
            GH_ObjectResponse val = this._composedCollection.RespondToMouseUp(sender, e);
            if ((int)val == 1)
            {
                this.ExpireLayout();
        ...
        {
            GH_ObjectResponse val = this._composedCollection.RespondToMouseDoubleClick(sender, e);
            if ((int)val != 0)
            {
                this.ExpireLayout();
        ...
        {
            GH_ObjectResponse val = this._composedCollection.RespondToKeyDown(sender, e);
            if ((int)val != 0)
            {
                this.ExpireLayout();
                ((Control)sender).Refresh();
        ...
        {
            GH_ObjectResponse val = this._composedCollection.RespondToMouseMove(sender, e);
            if ((int)val != 0)
            {
                this.ExpireLayout();
                ((Control)sender).Refresh();
        ...
        {
            GH_SwitchComponent component = (GH_SwitchComponent)Owner;

            //// Gets whether the switcher-component "Owning" this attribute is clicked
            ////      1. If not, set to clicked!!
            //if(!component.IsClicked) component.IsClicked = true;
            //// Set the "Clicked" status of the menu-collection "Owned by" the switcher-component 
            //this._composedCollection.IsOwnerClicked = component.IsClicked;

            // Compute the mouse-down function of the menus collection
            GH_ObjectResponse val = this._composedCollection.RespondToMouseDown(sender, e);

            if (val != GH_ObjectResponse.Ignore)
            {
                this.ExpireLayout();
                ((Control)sender).Refresh();
                return val;
        ...
        public override bool IsTooltipRegion(PointF pt)
        {
            ...
            {
                GH_CustomAttribute gH_CustomAttribute = collection.IsTtipPoint(pt);
                if (gH_CustomAttribute != null)
                {
                    _activeToolTip = gH_CustomAttribute;
                    return true;
                }
            }
            return false;
        ...
        {
            GH_CustomAttribute gH_Attr_Widget = this._composedCollection.IsTtipPoint(pt);
