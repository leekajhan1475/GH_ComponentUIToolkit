...
{
    class MenuRadioButton : GH_CustomAttribute
    {
		...
		{
			_active = false;
			_padding = WidgetServer.Default.RadioButtonPadding;
			_radioSize = WidgetServer.Default.RadioButtonSize;
			Tag = tag;
			RenderTag = true;
			Align = align;
		}
		...
		public override SizeF ComputeMinSize()
		{
			...
			if (Align == Alignment.Horizontal)
			{
				...
				{
					Size size = TextRenderer.MeasureText(Tag, WidgetServer.Default.TextFont);
			...
			else
			{
				...
				if (RenderTag && Tag != null)
				{
					Size size2 = TextRenderer.MeasureText(Tag, WidgetServer.Default.TextFont);
		...
		public override void Layout()
		{
			if (Align == Alignment.Horizontal)
			{
				...
				_radioBounds = new Rectangle((int)(base.CanvasPivot.X + base.CanvasBoundary.Width - (float)_radioSize.Width), (int)num, _radioSize.Width, _radioSize.Height);
			}
			else
			{
				float num2 = base.CanvasPivot.X + (base.Width - (float)_radioSize.Width) / 2f;
				_radioBounds = new Rectangle((int)num2, (int)(base.CanvasPivot.Y + base.CanvasBoundary.Height - (float)_radioSize.Height), _radioSize.Width, _radioSize.Height);
			}
		}
		...
		public override void Render(WidgetRenderArgs args)
		{
			...
			if (RenderTag && Tag != null)
			{
				if (Align == Alignment.Horizontal)
				{
					...
					graphics.DrawString(Tag, WidgetServer.Default.TextFont, solidBrush, point);
				}
				else
				{
					...
					graphics.DrawString(Tag, WidgetServer.Default.TextFont, solidBrush, point2, stringFormat);
				}
			}
			if (!_active)
			{
		...
		{
			if (base.CanvasBoundary.Contains(e.CanvasLocation))
			{
				_active = !_active;
			}
		...
		{
			if (base.CanvasBoundary.Contains(pt))
			{
				return this;
			}
			return null;
